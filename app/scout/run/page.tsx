    ...
    <div className="overflow-hidden bg-[var(--bg)] pt-16 text-[var(--ink)]">
      <section className="container-shell grid gap-14 py-20 md:grid-cols-[0.95fr_1.05fr] md:items-center md:py-28">
        <div className="min-w-0">
          <Link
            href="/"
            className="inline-flex items-center gap-2 text-sm font-semibold text-[var(--ink-dim)] transition hover:text-[var(--sun)]"
          >
          ...
          <p className="mt-7 max-w-[20rem] text-lg leading-8 text-[var(--ink-dim)] sm:max-w-2xl md:text-xl">
            This archived page documents the older Scout binary path. The
            current Orisan homepage leads with mcpscan and does not claim a curl
            install path for mcpscan.
          </p>
        </div>
        <Terminal />
      </section>
          ...
          <div>
            ...
            <div className="mt-10 grid border-l border-t border-[var(--rule)] md:grid-cols-3">
              {[
                ...
                <div
                  key={key}
                  className="border-b border-r border-[var(--rule)] p-6"
                >
                  <p className="font-mono text-[11px] uppercase tracking-[0.16em] text-[var(--sun)]">
                    {key}
                  </p>
                  <p className="mt-4 font-mono text-sm text-[var(--ink-dim)]">
                    {value}
                  </p>
                </div>
              ))}
            </div>
            <p className="mt-6 font-mono text-xs text-[var(--ink-faint)]">
              Explicit flags still work: {explicitCommand}
            </p>
        ...
        <div className="grid gap-10 md:grid-cols-[12rem_1fr]">
          ...
          <div>
            ...
            <p className="mt-6 max-w-2xl text-lg leading-8 text-[var(--ink-dim)]">
              Release artifacts include macOS, Linux, and Windows binaries with
              sha256 checksums. Exact versions belong here and in GitHub
              Releases, not in the primary user flow.
            </p>
            <div className="mt-10 grid border-l border-t border-[var(--rule)] md:grid-cols-3">
      ...
      <section className="container-shell py-20 md:py-28">
        ...
        <div className="grid border-l border-t border-[var(--rule)] md:grid-cols-4">
          {releaseSteps.map(([title, description]) => (
            <div
              key={title}
              className="min-h-40 border-b border-r border-[var(--rule)] p-6"
            >
              <h3 className="font-mono text-xs uppercase tracking-[0.16em] text-[var(--sun)]">
                {title}
              </h3>
              <p className="mt-6 break-words text-sm leading-6 text-[var(--ink-dim)]">
                {description}
              </p>
            </div>
          ))}
        ...
        <div className="grid border-l border-t border-[var(--rule)] md:grid-cols-3">
          {outputs.map(([title, description]) => (
            <div
              ...
              className="min-h-40 border-b border-r border-[var(--rule)] p-6"
            >
              <h3 className="font-mono text-xs uppercase tracking-[0.16em] text-[var(--ink)]">
                {title}
              </h3>
              <p className="mt-6 text-sm leading-6 text-[var(--ink-dim)]">
                {description}
              </p>
            </div>
          ))}
      ...
      <section className="container-shell py-20 md:py-28">
        <div className="border-y border-[var(--rule)] py-14 md:flex md:items-center md:justify-between md:gap-12">
          <div>
            ...
            <p className="mt-6 max-w-2xl text-lg leading-8 text-[var(--ink-dim)]">
              Do not share source code, secrets, or private reports unless your
              policy allows it. Finding count, usefulness, noise, and missing
              coverage are enough.
            </p>
          </div>
          <a
      ...
      <section className="container-shell py-20 md:py-28">
        <div className="grid gap-10 md:grid-cols-[12rem_1fr]">
          ...
          <div>
            <h2 className="max-w-2xl text-3xl font-semibold leading-tight tracking-[-0.03em] md:text-5xl">
              v0.1 checks repo-local MCP configs and repo-level agent
              instructions.
            </h2>
            <div className="mt-10 flex flex-wrap gap-3">
              {scope.map((item) => (
                <span
                  key={item}
                  className="border border-[var(--rule)] px-3 py-2 font-mono text-xs text-[var(--ink-dim)]"
                >
                  {item}
                </span>
              ))}
