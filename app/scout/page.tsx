    ...
    <div className="overflow-hidden bg-[var(--bg)] pt-16 text-[var(--ink)]">
      <section className="container-shell grid min-w-0 gap-14 py-20 md:grid-cols-[1fr_0.95fr] md:items-center md:py-28">
        <div className="min-w-0">
          ...
          <p className="mt-7 max-w-[21rem] text-lg leading-8 text-[var(--ink-dim)] sm:max-w-2xl md:text-xl">
            Scout is kept as a secondary community artifact. The current Orisan
            flagship is mcpscan for local MCP server review.
          </p>
          <p className="mt-5 max-w-[21rem] font-mono text-xs uppercase leading-6 tracking-[0.13em] text-[var(--ink-faint)] sm:max-w-2xl">
            Local by default. No source upload. No cloud upload. Deterministic
            checks.
          </p>
        ...
        <div className="grid min-w-0 gap-10 md:grid-cols-[12rem_1fr]">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-10 grid min-w-0 border-l border-t border-[var(--rule)] md:grid-cols-3">
              {quickStart.map(([title, body]) => (
                ...
                >
                  ...
                  <p className="mt-6 break-words font-mono text-sm leading-7 text-[var(--ink-dim)] [overflow-wrap:anywhere]">
                    {body}
                  </p>
                </div>
              ))}
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link
                href="/scout/run"
                className="bg-[var(--ink)] px-5 py-3 text-center font-mono text-xs font-semibold uppercase tracking-[0.1em] text-[var(--bg)] transition hover:bg-[var(--sun)]"
              >
                Full runbook
              </Link>
              <Link
                href="/scout/sample-report"
                className="border-b border-[var(--rule-2)] px-1 py-3 text-center font-mono text-xs font-semibold uppercase tracking-[0.1em] text-[var(--ink)] transition hover:border-[var(--sun)] hover:text-[var(--sun)]"
              >
                Sample report
              </Link>
              <a
      ...
      <section className="container-shell py-20 md:py-28">
        <div className="grid gap-10 md:grid-cols-[12rem_1fr]">
          ...
          <div>
            <h2 className="max-w-2xl text-3xl font-semibold leading-tight tracking-[-0.03em] md:text-5xl">
              Scout turns repo-local agent exposure into questions a reviewer
              can answer.
            </h2>
      ...
      <section className="container-shell py-20 md:py-28">
        ...
        <div className="grid border-l border-t border-[var(--rule)] md:grid-cols-3">
          {audience.map(([title, body]) => (
            <div
              key={title}
              className="min-h-52 border-b border-r border-[var(--rule)] p-6"
            >
              <h3 className="font-mono text-xs uppercase tracking-[0.16em] text-[var(--sun)]">
                {title}
              </h3>
        ...
        <div className="grid gap-10 border-y border-[var(--rule)] py-14 md:grid-cols-[0.9fr_1.1fr]">
          ...
          <div>
            <p className="text-lg leading-8 text-[var(--ink-dim)]">
              Current Orisan positioning leads with mcpscan. Scout remains
              available for context on repo-local agent approval experiments,
              but it should not be treated as the main product.
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <a
