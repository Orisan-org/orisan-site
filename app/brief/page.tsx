    ...
    <div className="bg-[var(--bg)] pt-16 text-[var(--ink)]">
      <section className="container-shell grid gap-14 py-20 md:grid-cols-[1fr_0.95fr] md:items-end md:py-28">
        ...
        <p className="text-lg leading-8 text-[var(--ink-dim)] md:text-xl">
          This page is retained as background. The current Orisan homepage and
          public story lead with mcpscan, the local-first MCP server security
          scanner.
        </p>
      </section>
        ...
        <div className="grid gap-10 md:grid-cols-[12rem_1fr]">
          ...
          <div>
            ...
            <p className="mt-6 max-w-3xl text-lg leading-8 text-[var(--ink-dim)]">
              Security teams already know how to review code, dependencies,
              secrets, and infrastructure. What is newer is the agent layer:
              local tool access, MCP config, inherited instructions, and
              repository-specific behavior that may authorize reading,
              execution, or changes before anyone has written a new line of
              code.
            </p>
          </div>
        </div>
      ...
      <section className="container-shell py-20 md:py-28">
        ...
        <div className="grid border-l border-t border-[var(--rule)]">
          {validationQuestions.map((question, index) => (
            <div
              key={question}
              className="grid gap-5 border-b border-r border-[var(--rule)] p-5 md:grid-cols-[4rem_1fr]"
            >
          ...
          <div>
            ...
            <div className="mt-10 grid border-l border-t border-[var(--rule)] md:grid-cols-4">
              {realRunProtocol.map(([title, body]) => (
                <div
                  key={title}
                  className="min-h-44 border-b border-r border-[var(--rule)] p-6"
                >
                  <h3 className="font-mono text-xs uppercase tracking-[0.16em] text-[var(--sun)]">
                    {title}
                  </h3>
                  <p className="mt-8 break-words text-sm leading-7 text-[var(--ink-dim)]">
                    {body}
                  </p>
                </div>
              ))}
            ...
            <p className="mt-8 max-w-2xl text-sm leading-7 text-[var(--ink-faint)]">
              Do not use this archived Scout brief as the current product
              runbook.
            </p>
          </div>
        </div>
            ...
            <div>
              ...
              <div className="mt-8 grid border-l border-t border-[var(--rule)]">
                {successSignals.map((signal) => (
                  <div
                    key={signal}
                    className="border-b border-r border-[var(--rule)] p-5 text-sm leading-7 text-[var(--ink-dim)]"
                  >
                    {signal}
                  </div>
                ))}
