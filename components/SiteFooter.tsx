  ...
  return (
    <footer className="border-t border-grey-3">
      <div className="mx-auto flex max-w-wrap flex-wrap items-baseline justify-between gap-6 px-8 pb-13 pt-8">
        ...
        <div className="flex flex-wrap items-baseline gap-6 font-mono text-micro uppercase tracking-meta text-grey-2">
          {LINKS.map((l) => (
            <Link key={l.href} href={l.href} className="text-grey-2 no-underline">
              {l.label}
            </Link>
          ))}
          <a href="mailto:team@orisan.org" className="text-grey-2 no-underline">
            team@orisan.org
          </a>
          <span>&copy; 2026</span>
        </div>
